...
def _qualify_stats(graph: Mapping[str, Any], evidence: PlanEvidence) -> int:
    ...
    for edge in graph["edges"]:
        column = f"j_{edge['id']}"
        expected[(f"r{edge['left']:04d}", column)] = edge["left_key_ndv"]
        expected[(f"r{edge['right']:04d}", column)] = edge["right_key_ndv"]
    if set(records) != set(expected):
        raise QualificationError("pg_stats columns do not exactly match graph edge columns")
    for key, key_ndv in expected.items():
        record = records[key]
        if float(record["null_frac"]) != 0.0 or int(record["avg_width"]) != 8:
            raise QualificationError(f"uniform stats width/null contract differs for {key}")
