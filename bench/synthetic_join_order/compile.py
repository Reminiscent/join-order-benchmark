...
or a predetermined join order. One projected payload column per relation
carries the generated physical width through join costing.
"""
...
# This is a controlled costing unit, not PostgreSQL heap-layout emulation.
VIRTUAL_PAGE_BYTES = 8_192
...
def _compile_graph_markdown(graph: Mapping[str, Any]) -> str:
    ...
    for node in graph["nodes"]:
        anchor = " (anchor)" if node["id"] == graph["cardinality_model"]["anchor_relation"] else ""
        lines.append(
            f'    R{node["id"]}["R{node["id"]}{anchor}<br/>'
            f'rows={node["base_rows"]:,}, payload={node["payload_width"]}B"]'
        )
    for edge in graph["edges"]:
        lines.append(
            f'    R{edge["left"]} ---|"{edge["growth_role"]}<br/>'
            f'growth ×{_compact_number(edge["growth_factor"])}"| R{edge["right"]}'
        )
    lines.extend(
        [
            "```",
            ...
            "",
            "| Edge | Direction | Role | Growth | Left `D` | Left `M` | "
            "Right `D` | Right `M` | Overlap `q` | Selectivity |",
            "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    for edge in graph["edges"]:
        ...
        lines.append(
            ...
            f'| R{edge["parent"]} → R{edge["child"]} '
            f'| `{edge["growth_role"]}` '
            f'| {_compact_number(edge["growth_factor"])} '
            f'| {edge["left_key_ndv"]:,} '
...
def relation_page_counts(graph: Mapping[str, Any]) -> dict[int, int]:
    """Return virtual pages from rows and projected tuple width.
...
def _compile_schema(graph: Mapping[str, Any], schema_name: str) -> str:
    ...
    for node in graph["nodes"]:
        ...
        columns = ["payload text"] + [
            f"{_quote_identifier(_column_name(edge))} bigint"
    ...
    for node in graph["nodes"]:
        ...
        lines.extend(
            [
                ...
                "        'null_frac', 0::real,",
                f"        'avg_width', {node['payload_width']}::integer,",
                "        'n_distinct', -1::real) THEN",
                f"        RAISE EXCEPTION 'failed to restore payload stats for {relation_name}';",
                "    END IF;",
            ]
        )
        for edge in sorted(incident[node["id"]], key=lambda value: value["id"]):
...
def _compile_query(graph: Mapping[str, Any], schema_name: str) -> str:
    ...
    targets = [
        f'    {_quote_identifier(_relation_name(node["id"]))}."payload" '
        f'AS "payload_{node["id"]:04d}"'
        for node in graph["nodes"]
    ]
    predicates = []
    for edge in graph["edges"]:
