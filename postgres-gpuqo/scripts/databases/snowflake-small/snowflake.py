...
if __name__ == "__main__":
    ...
    # For large experiments raise GPUQO_MIN_ROWS/GPUQO_MAX_ROWS.
    min_rows = int(environ.get("GPUQO_MIN_ROWS", "20"))
    max_rows = int(environ.get("GPUQO_MAX_ROWS", "200"))
    max_query_size = int(environ.get("GPUQO_MAX_QUERY_SIZE", "40"))
    queries_per_size = int(environ.get("GPUQO_QUERIES_PER_SIZE", "10"))

    # 1 fact table -> 16 level-2 -> 8 level-3 -> 4 level-4.
    schema = SnowflakeSchema((1, 16, 8, 4, 0), (0, 8, 4, 4, 0))
    ...
    with open("load.sql", "w") as f:
        schema.write_insert_into(f, min_rows, max_rows)
    ...
    # Query count: max_query_size * queries_per_size.
    for n in tqdm(range(2, max_query_size + 1)):
